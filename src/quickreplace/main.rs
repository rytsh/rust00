...
fn main() {
    ...
    let data = match fs::read_to_string(&args.filename) {
        Ok(content) => content,
        Err(e) => {
            eprintln!("{} could not read file '{}': {}", "Error:".red().bold(), args.filename, e);
            std::process::exit(1);
        }
    };
    ...
    let replaced_data = match replace(&args.target, &args.replacement, &data) {
        Ok(result) => result,
        Err(e) => {
            eprintln!("{} regex error: {}", "Error:".red().bold(), e);
            std::process::exit(1);
        }
    };
    ...
    match fs::write(&args.output, &replaced_data) {
        Ok(_) => {}
        Err(e) => {
            eprintln!("{} could not write file '{}': {}", "Error:".red().bold(), args.output, e);
            std::process::exit(1);
        }
    };
}
